...
developmentChains[0] !== "localhost"
  ...
  : describe("Dynamic SVG NFT Tests", () => {
      let DynamicSvgNft, MockV3Aggregator, accounts, minter;
      ...
      beforeEach(async () => {
        ...
        let DynamicSvgNftContract = await ethers.getContract("DynamicSvgNft");
        DynamicSvgNft = DynamicSvgNftContract.connect(minter);
      });

      describe("constructor", () => {
        it("Sets the tokenCounter correctly", async () => {
        ...
        it("Sets the ImageURI correctly", async () => {
      ...
      describe("mint", () => {
        it("Mints a new NFT and Emits an Event", async () => {
          await expect(
            DynamicSvgNft.mintNft(ethers.utils.parseEther("1"))
